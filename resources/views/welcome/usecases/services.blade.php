<div style="margin: 10vh 0; max-width: 1080px;" class="d-block mx-auto" id="services_section">
    <br>
    <h2 class="fw-bold" style="font-size:50px;">What We Serve</h2>
    <div class="row mt-4">
        <div class="col-lg-4 col-md-6 col-sm-12 col-12 mx-auto">
            <div class="container-landing bg-success">
                <i class="fa-solid fa-car mb-4" style="font-size:34px;"></i>
                <h3>Vehicle Listing</h3>
                <p>Manage and view all registered vehicles with complete details</p>
            </div>
        </div>
        <div class="col-lg-4 col-md-6 col-sm-12 col-12 mx-auto">
            <div class="container-landing bg-success">
                <i class="fa-solid fa-gears mb-4" style="font-size:34px;"></i>
                <h3>Service Schedule</h3>
                <p>Plan and track vehicle maintenance schedules efficiently</p>
            </div>
        </div>
        <div class="col-lg-4 col-md-6 col-sm-12 col-12 mx-auto">
            <div class="container-landing bg-success">
                <i class="fa-solid fa-broom mb-4" style="font-size:34px;"></i>
                <h3>Wash Schedule</h3>
                <p>Schedule and monitor vehicle washing tasks with ease</p>
            </div>
        </div>
        <div class="col-lg-4 col-md-6 col-sm-12 col-12 mx-auto">
            <div class="container-landing bg-success">
                <i class="fa-solid fa-suitcase mb-4" style="font-size:34px;"></i>
                <h3>Trip History</h3>
                <p>View past trips with timestamps, routes, and person with</p>
            </div>
        </div>
        <div class="col-lg-4 col-md-6 col-sm-12 col-12 mx-auto">
            <div class="container-landing bg-success">
                <i class="fa-solid fa-user mb-4" style="font-size:34px;"></i>
                <h3>Manage Driver</h3>
                <p>Add, update, and monitor driver details and assignments</p>
            </div>
        </div>
        <div class="col-lg-4 col-md-6 col-sm-12 col-12 mx-auto">
            <div class="container-landing bg-success">
                <i class="fa-solid fa-chart-simple mb-4" style="font-size:34px;"></i>
                <h3>Statistic</h3>
                <p>Analyze vehicle usage, history, and efficiency data</p>
            </div>
        </div>
        <div class="col-lg-4 col-md-6 col-sm-12 col-12 mx-auto">
            <div class="container-landing bg-success">
                <i class="fa-solid fa-location-dot mb-4" style="font-size:34px;"></i>
                <h3>Live Tracking</h3>
                <p>Track vehicle locations in real-time with GPS integration</p>
            </div>
        </div>
    </div>
</div>