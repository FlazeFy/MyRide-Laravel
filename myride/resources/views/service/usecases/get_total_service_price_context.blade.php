...
<script>
    const get_total_price_service_by_category = () => {
        Swal.showLoading()
        const title = 'Total Service Price By Category'
        const ctx = 'total_price_service_by_category_temp'
        const ctx_holder = 'stats_total_price_service_by_category_holder'
        const type_chart =  '<?= session()->get('toogle_total_stats') ?>'

        const fetchData = () => {
            $.ajax({
                url: `/api/v1/stats/total/service/service_category`,
                type: 'GET',
    ...
    get_total_price_service_by_category()
</script>